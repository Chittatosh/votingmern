...
class VoteComponent extends React.Component {
  ...
  render() {
    ...
    const itsAlreadyThere =
      newChoiceTrimmed &&
      choiceNameArr.find(
        choiceName => choiceName.toLowerCase() === newChoiceTrimmed,
      );
    const newSubmit = (
    ...
    const voteNdelete = deleteBool ? (
      <div className="container">
        <div className="row">
          <div className="col-sm">{voteButton}</div>
          <div className="col-sm">{deleteButton}</div>
        </div>
      </div>
    ) : (
      voteButton
        ...
        <div className="form-group">
          ...
          >
            ...
            {choiceNameArr.map((choiceName, index) => (
              <option key={choiceName} value={index + 1}>
                {choiceName}
              </option>
            ))}
            {fbggId && (
              <option value="new">Add a new choice and vote for it</option>
            )}
          </select>
        </div>
        {this.state.choiceNum === 'new' ? newSubmit : voteNdelete}
        {isFetching.slice(5) === _id &&
          notification(`${isFetching.slice(0, 5)}ing...`)}
        {fetchError === _id &&
          notification('You have already voted in this Poll!')}
...
VoteComponent.propTypes = {
  ...
  thunkVote: PropTypes.func.isRequired,
  thunkDelete: PropTypes.func.isRequired,
};
