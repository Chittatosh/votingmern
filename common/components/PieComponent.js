...
class PieComponent extends React.Component {
  ...
  d3run() {
    ...
    labels.attr('class', 'label')
      ...
      .text(d => {
        if (d.data.voteCount && (d.data.voteCount * 100 / voteSum) >= 5)
          return `${d.data.choiceName.length > 14 ? d.data.choiceName.substring(0,11)+'...' : d.data.choiceName}`;
        else
          return null;
      });
    ...
    path
      .on('mouseover', d => {
        ...
        tooltip
          .html(`${d.data.choiceName}<br/>Votes: ${d.data.voteCount} (${+(d.data.voteCount * 100 / voteSum).toFixed(2)}%)`)
...
PieComponent.propTypes = {
  ...
  choiceArr: PropTypes.arrayOf(PropTypes.shape({
