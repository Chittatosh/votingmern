...
const getUrl = endpoint => url.resolve(APIURL, endpoint);
...
export const fetchInitiate = fetchId => ({
  type: types.FETCH_INITIATE,
  fetchId,
});
...
export const thunkVote = (_id, { choiceNum, newChoice }) => dispatch => {
  ...
  return fetch(getUrl(endpoint + _id), getInit('PUT', choiceBody))
    .then(res => res.json())
    .then(body => {
      if (body.repeatVote) dispatch(fetchFailure(body.repeatVote));
      else dispatch(fetchSuccess(body));
    })
    .catch(err => dispatch(fetchFailure(err.message)));
};
